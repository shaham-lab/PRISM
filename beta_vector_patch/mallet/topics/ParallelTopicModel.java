...
public class ParallelTopicModel implements Serializable {
	...
	public String betaPath; // Path to a file containing beta values

	public boolean usingSymmetricAlpha = false;
	public boolean usingSymmetricBeta = false;
	...
	public ParallelTopicModel (LabelAlphabet numberOfTopics, double alphaSum, double beta) {
		// this (newLabelAlphabet (numberOfTopics), alphaSum, beta);
	}
	...
	/**
	 * Loads a beta prior vector from a file. The file should contain a single row with numTypes comma-separated values.
	 *
	 * @param filePath Path to the beta prior file.
	 * @param numTypes Number of word types (vocabulary size).
	 * @return A double array of length numTypes representing the beta prior.
	 * @throws IOException If file format is incorrect or reading fails.
	 */
	public double[] loadBetaPrior(String filePath, int numTypes) throws IOException {
		System.out.println("Loading beta prior vector from file: " + filePath);
		System.out.println("Expected vector length (numTypes): " + numTypes);

		try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
			String line = reader.readLine();

			if (line == null) {
				throw new IOException("Beta prior file is empty.");
			}

			String[] values = line.trim().split(",");
			System.out.println("Sample beta vector: " + String.join(" ", Arrays.copyOf(values, Math.min(10, values.length))) + "...");

			if (values.length != numTypes) {
				throw new IOException("Mismatch in numTypes. Got " + values.length + " values instead of expected " + numTypes);
			}

			double[] betaVector = new double[numTypes];
			for (int i = 0; i < numTypes; i++) {
				try {
					betaVector[i] = Double.parseDouble(values[i]);
				} catch (NumberFormatException e) {
					throw new IOException("Invalid number at index " + i + ": " + values[i], e);
				}
			}

			System.out.println("Successfully loaded beta prior vector.");
			return betaVector;
		} catch (IOException e) {
			System.err.println("Error reading beta prior file: " + e.getMessage());
			throw e;
		}
	}

	public ParallelTopicModel (LabelAlphabet topicAlphabet, double alphaSum, String betaPath) {
		...
		System.err.println("# betaPath: " + betaPath);
	...
	public void addInstances (InstanceList training) {
		...
		try {
			this.beta = loadBetaPrior(this.betaPath, this.numTypes);
		} catch (IOException e) {
			throw new RuntimeException("Failed to load beta matrix from file: " + betaPath, e);
		}
		...
		this.betaSum = roundToFiveDecimals(betaSum); // round to 5 decimals
		System.err.println("betaSum: " + betaSum);
		Randoms random = null;
		if (randomSeed == -1) {
			random = new Randoms();
		}
		...
		for (Instance instance : training) { // training - containes all documents as tokens.
		...
		while (line.startsWith("#")) {
			if (line.startsWith("#alpha : ")) {
				...
				for (int type = 0; type < fields.length; type++) {
					this.beta[type] = Double.parseDouble(fields[type]);
					this.betaSum += this.beta[type];
				}
			}
	...
	public void buildInitialTypeTopicCounts () {
		...
		// for each type, we have the total number of times it occurs in the corpus - save it into file
		// saveToFile("Words",typeTotals);
	...
	public void updateOptimizedBeta(WorkerRunnable[] runnables) {
		int[][] countHistogram = new int[numTypes][maxTypeCount + 1]; // Counts how many times words appear in topics.
			
		// Now count the number of type/topic pairs that have
		//  each number of tokens.
		// countHistogram - for each word, which topics are assigned to it and how many times.
		int index;
		for (int type = 0; type < numTypes; type++) {
		...
		//  histogram.
		int maxTopicSize = 0; // Finds the largest topic size
		for (int topic = 0; topic < numTopics; topic++) {
		...
		// Builds a histogram (topicSizeHistogram) that tracks topic-word distributions.
		// topicSizeHistogram - for each topic, which words are assigned to it and how many times.
	...
	/** 
	 *  Optimizing Beta dirichlet prior
	 */
	public void optimizeBeta(WorkerRunnable[] runnables) {
		if (! usingSymmetricBeta) {
			// @@@@@@@@@@@@@@@@@@@@@@@@@@@
			// FISRST TRY
			// @@@@@@@@@@@@@@@@@@@@@@@@@@@

			// int[] topicLengthCounts = new int[maxTypeCount + 1];  // where maxTokensPerTopic = max(tokensPerTopic[k])
			// int[][] tokenTopicCounts = new int[numTypes][maxTypeCount + 1]; // maxTypeTopicCount = max times any word appears in a topic

			// // Step 1: topicLengthCounts[k] = how many topics have length k
			// int maxTopicSize = 0;
			// for (int k = 0; k < numTopics; k++) {
			// 	int topicSize = tokensPerTopic[k];
			// 	if (topicSize > maxTopicSize) {
			// 		maxTopicSize = topicSize;
			// 	}
			// 	topicLengthCounts[topicSize]++;
			// }

			// // Step 2: tokenTopicCounts[w][c] = how many topics assigned word w exactly c times
			// for (int w = 0; w < numTypes; w++) {
			// 	int[] counts = typeTopicCounts[w];
			// 	int index = 0;

			// 	while (index < counts.length && counts[index] > 0) {
			// 		int topic = counts[index] & topicMask;
			// 		int count = counts[index] >>> topicBits; // MALLET stores count << topicBits + topic

			// 		tokenTopicCounts[w][count]++;
			// 		index++;
			// 	}
			// }
			...
			// int[] tempTokensPerTopic = new int[numTopics];
			// int[][] tempTypeTopicCounts = new int[numTypes][numTopics];
			// Arrays.fill(tempTokensPerTopic, 0);
			// for (int type = 0; type < tempTypeTopicCounts.length; type++) {
			// 	Arrays.fill(tempTypeTopicCounts[type], 0);
			// }

			// for (int thread = 0; thread < numThreads; thread++) {
			// 	int[] sourceLengthCounts = runnables[thread].getTokensPerTopic();
			// 	int[][] sourceTypeCounts = runnables[thread].getTypeTopicCounts();

			// 	for (int count=0; count < sourceLengthCounts.length; count++) {
			// 		if (sourceLengthCounts[count] > 0) {
			// 			tempTokensPerTopic[count] += sourceLengthCounts[count];
			// 			sourceLengthCounts[count] = 0;
			// 		}
			// 	}
			...
			// 		if (! usingSymmetricBeta) {
			// 			for (int count=0; count < sourceTypeCounts[type].length; count++) {
			// 				if (sourceTypeCounts[type][count] > 0) {
			// 					tempTypeTopicCounts[type][count] += sourceTypeCounts[type][count];
			// 					sourceTypeCounts[type][count] = 0;
			// 				}
			// 			}
			// 		}
			// 		else {
			...
			// 			for (int count=0; count < sourceTypeCounts[type].length; count++) {
			// 				if (sourceTypeCounts[type][count] > 0) {
			// 					tempTypeTopicCounts[0][count] += sourceTypeCounts[type][count];
			// 					//			 ^ the only change
			// 					sourceTypeCounts[type][count] = 0;
			// 				}
			// 			}
			...
			int[][] countHistogram = new int[numTypes][maxTopicSize + 1]; // Counts how many times words appear in topics.
			...
			try {
				betaSum = Dirichlet.learnParameters(beta, countHistogram, topicSizeHistogram, 1.001, 1.0, 1);
				// Arrays.fill(tokensPerTopic, 0);
				// // for (int type = 0; type < typeTopicCounts.length; type++) {
				// // 	Arrays.fill(typeTopicCounts[type], 0);
				// // }
				// // tokensPerTopic = tempTokensPerTopic;
				// // typeTopicCounts = tempTypeTopicCounts;
			} catch (RuntimeException e) {
				// Dirichlet optimization has become unstable. This is known to happen for very small corpora (~5 docs).
				logger.warning("Dirichlet optimization has become unstable. Resetting to beta_t = 0.01");
				betaSum = numTypes * 0.01;
				for (int type = 0; type < numTypes; type++) {
					beta[type] = 0.01;
				}
			}
			...
			int[] countHistogram = new int[maxTypeCount + 1]; // Counts how many times words appear in topics.
			...
			double betaValue = betaSum / numTypes;
			Arrays.fill(beta, betaValue);
	...
	public void saveToFile(String typeSave, int[] assignments) {
		if (typeSave == "Topics") {
			try (FileWriter fileWriter = new FileWriter("tokens_per_topic.txt", false);
				PrintWriter printWriter = new PrintWriter(fileWriter)) {
			
				for (int i = 0; i < assignments.length; i++) {
					printWriter.printf("Topic: %d  Token: %d%n", i, assignments[i]);
				}
			
				printWriter.flush();
				System.err.println("Topic assignments saved to tokens_per_topic.txt");
			} catch (IOException e) {
				System.err.println("Error writing topic assignments to file: " + e.getMessage());
			}

		} else if (typeSave == "Words") {
			try (FileWriter fileWriter = new FileWriter("words_total.txt", false);
				PrintWriter printWriter = new PrintWriter(fileWriter)) {
			
				for (int i = 0; i < assignments.length; i++) {
					printWriter.printf("Type: %d  Total: %d%n", i, assignments[i]);
				}
			
				printWriter.flush();
				System.err.println("Word assignments saved to words_total.txt");
			} catch (IOException e) {
				System.err.println("Error writing word assignments to file: " + e.getMessage());
			}
		}
		else {
			System.err.println("Unknown type for saving assignments: " + typeSave);
		}
		
	}
	/** Runs Collapsed Gibbs Sampling (CGS) to infer topic distributions over documents and words.
	 * This method:
		* Initializes and distributes work among multiple threads.
		* Iterates Gibbs Sampling for numIterations times.
		* Periodically saves model states and logs likelihood values.
		* Optionally optimizes hyperparameters α and β.
	 * 
	 */
	public void estimate () throws IOException {
		...
		System.err.println("Finished " + numIterations + " iterations of CGS.");
		// save tokenPerTopic
		// saveToFile("Topics", tokensPerTopic);
		...
		while (iteration < iterations && totalChange > 0) {
			...
			for (int doc = 0; doc < data.size(); doc++) {
				...
				for (int topic = 0; topic < numTopics; topic++) {
					...
					if (topicCoefficients[topic] > globalMaxScore) {
						globalMaxTopic = topic;
						globalMaxScore = topicCoefficients[topic];
					}
				}
				...
				for (int position = 0; position < docLength; position++) {
					...
					if (oldTopic == globalMaxTopic) {
						globalMaxScore = beta[type] * topicCoefficients[oldTopic];
						for (int topic = 0; topic < numTopics; topic++) {
							if (beta[type] * topicCoefficients[topic] > globalMaxScore) {
								globalMaxTopic = topic;
								globalMaxScore = beta[type] * topicCoefficients[topic];
							}
						}
					}
					...
					while (index < currentTypeTopicCounts.length && 
						...
						if (! alreadyDecremented && currentTopic == oldTopic) {
							...
							score = 
								topicCoefficients[currentTopic] * (beta[type] + currentValue);
							if (score > maxScore) {
					...
					if (beta[type] * topicCoefficients[newTopic] > globalMaxScore) {
						globalMaxScore = beta[type] * topicCoefficients[newTopic];
			...
			for (int type = 0; type < numTypes; type++) {
				...
				while (index < topicCounts.length &&
					...
					if (currentTopic == topic) {
						weight[currentTopic] += topicCounts[index] >> topicBits;
						break;
					}
	...
	public double[][] getSubCorpusTopicWords(boolean[] documentMask, boolean normalized, boolean smoothed) {		
		...
		if (normalized) {
			...
			if (smoothed) {
				...
				for (int topic = 0; topic < numTopics; topic++) {
					topicNormalizers[topic] = 1.0 / (subCorpusTokensPerTopic[topic] + betaSum);
				}
			}
			else {
				for (int topic = 0; topic < numTopics; topic++) {
	...
	public double[][] getTopicWords(boolean normalized, boolean smoothed) {
		...
		if (normalized) {
			...
			if (smoothed) {
				...
				for (int topic = 0; topic < numTopics; topic++) {
					topicNormalizers[topic] = 1.0 / (tokensPerTopic[topic] + betaSum);
				}
			}
			else {
				for (int topic = 0; topic < numTopics; topic++) {
	...
	public double modelLogLikelihood() {
		...
		int nonZeroTypeTopics = 0; // counts how many (topic, word) pairs have non-zero counts.
		...
		for (int type=0; type < numTypes; type++) {
			...
			// implement ∑_k ∑_w log Γ(n_{k,w} + β) :
			int index = 0;
			while (index < topicCounts.length &&
				   topicCounts[index] > 0) {
				int topic = topicCounts[index] & topicMask;
				...
				nonZeroTypeTopics++;
				logLikelihood += Dirichlet.logGammaStirling(beta[type] + count);
		...
		// SECTION 3: Subtract logGamma of normalizer per topic
		for (int topic=0; topic < numTopics; topic++) {
			// log Γ(∑_w β + N_k)
			logLikelihood -= 
				Dirichlet.logGammaStirling( (betaSum) +
											tokensPerTopic[ topic ] );
		...
		// Compute log Γ(∑_w β_w), which is the normalization constant in Dir(β)
		// It stays the same for every topic since all topics share the same β vector
		double logGammaTopicBetaSum = Dirichlet.logGammaStirling(betaSum);

		// Now compute the Dirichlet prior normalization contribution per topic
		for (int topic = 0; topic < numTopics; topic++) {
			// Add log Γ(∑_w β_w) for this topic
			logLikelihood += logGammaTopicBetaSum;

			// Subtract ∑_w log Γ(β_w)
			// These terms correspond to the denominator in the Dirichlet PDF:
			// B(β) = Γ(∑ β_w) / ∏_w Γ(β_w)
			for (int type = 0; type < numTypes; type++) {
				logLikelihood -= Dirichlet.logGammaStirling(beta[type]);
			}
		}
	...
	public MarginalProbEstimator getProbEstimator() {
		return new MarginalProbEstimator(numTopics, numTypes, alpha, alphaSum, beta, betaSum,
										 typeTopicCounts, tokensPerTopic);
	}
	...
	public static void main (String[] args) {
		...
		try {
			...
			ParallelTopicModel lda = new ParallelTopicModel (topicAlphabet, 50.0, "/home/tal/dev/Mallet/priors/transpose_23205.csv");
